...
<main class="md-content data-explorer">
  <section class="md-card md-elev-2" style="padding:1rem;">
    ...
    <form method="post" class="explorer-filters">
      ...
      <label class="md-field"><span><?=t($t, 'department', 'Directorate')?></span><select name="department"><option value=""><?=t($t, 'all_departments', 'All directorates')?></option><?php foreach ($departmentOptions as $value): ?><option value="<?=htmlspecialchars($value, ENT_QUOTES, 'UTF-8')?>" <?=$scopeFilters['department'] === $value ? 'selected' : ''?>><?=htmlspecialchars($value, ENT_QUOTES, 'UTF-8')?></option><?php endforeach; ?></select></label>
      <label class="md-field"><span><?=t($t, 'cadre', 'Team in the Directorate')?></span><select name="team"><option value=""><?=t($t, 'all_teams', 'All teams')?></option><?php foreach ($teamOptions as $value): ?><option value="<?=htmlspecialchars($value, ENT_QUOTES, 'UTF-8')?>" <?=$scopeFilters['team'] === $value ? 'selected' : ''?>><?=htmlspecialchars($value, ENT_QUOTES, 'UTF-8')?></option><?php endforeach; ?></select></label>
  ...
  <section class="md-card md-elev-2" style="padding:1rem;margin-top:1rem;">
    ...
    <div style="overflow:auto;">
      ...
      <?php endif; ?>
      <table class="md-table"><thead><tr><th>ID</th><th><?=t($t, 'questionnaire', 'Questionnaire')?></th><th><?=t($t, 'individual', 'Individual')?></th><th><?=t($t, 'role', 'Role')?></th><th><?=t($t, 'directorate', 'Directorate')?></th><th><?=t($t, 'department', 'Directorate')?></th><th><?=t($t, 'cadre', 'Team in the Directorate')?></th><th><?=t($t, 'work_function', 'Work Role')?></th><th><?=t($t, 'completion_level', 'Completion level')?></th><th><?=t($t, 'created_at', 'Created')?></th></tr></thead>
